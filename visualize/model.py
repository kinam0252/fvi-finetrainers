...
# from .attn_processor import CogVideoXAttnProcessor2_0
...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, CacheMixin):
    ...
    def forward(
        ...
        if self.ofs_embedding is not None:
            raise NotImplementedError("OFs embedding is not implemented")
        ...
        # apply_target_noise_only가 True일 때 마지막 프레임 처리를 위한 마스크 생성
        if apply_target_noise_only:
            if apply_target_noise_only == "back":
                print("apply_target_noise_only: back")
                # 마지막 프레임 위치 식별
                target_frame_indices = torch.arange(num_frames, device=hidden_states.device) == (num_frames - 1)
            elif apply_target_noise_only == "front":
                print("apply_target_noise_only: front")
                # 첫 번째 프레임 위치 식별
                target_frame_indices = torch.arange(num_frames, device=hidden_states.device) == 0
            elif apply_target_noise_only == "front-long":
                print("apply_target_noise_only: front-long")
                # 첫 번째부터 6번째 프레임까지 위치 식별
                target_frame_indices = torch.arange(num_frames, device=hidden_states.device) < 6
            elif apply_target_noise_only == "front-last-long":
                print("apply_target_noise_only: front-last-long")
                # 첫 번째부터 6번째 프레임까지 위치 식별
                target_frame_indices = (torch.arange(num_frames, device=hidden_states.device) < 6) | (torch.arange(num_frames, device=hidden_states.device) == num_frames - 1)
        ...
        # 4. Final block
        # apply_target_noise_only가 True일 때 타겟 프레임은 timestep 0으로 처리
        if apply_target_noise_only and target_frame_mask is not None:
            # 타겟 프레임만 timestep 0으로 처리
            hidden_states_target = self.norm_out(hidden_states, temb=zero_emb)
            hidden_states_others = self.norm_out(hidden_states, temb=emb)
            
            # 마스크를 사용하여 결합
            hidden_states = torch.where(
                target_frame_mask.unsqueeze(-1), 
                hidden_states_target, 
                hidden_states_others
            )
        else:
        ...
        return Transformer2DModelOutput(sample=output, hidden_states=hidden_states_list)
